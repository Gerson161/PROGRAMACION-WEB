...
// AQUI EL USUARIO Y ADMIN PIDEN LA LISTA DE LIBROS 
router.get("/", verificarToken, async(req, res) => {
    const buscar = req.query.buscar || "";
    const where = buscar ? { titulo: {
            [Op.like]: `%${buscar}%` } } : {};
    const libros = await Libro.findAll({ where, order: [
            ["id", "DESC"]
        ] });
    res.json(libros);
});

// AQUI EL ADMIN CREA LIBROS 
